function ausgabenFolderScannen(rootFolderId, month) {
...
function wennBelegNeuIstEintragen(beleg, datum, BM) {
    ...
    var belegWoerter = beleg.getName().split(" ");
    ...
    var neueAusgabeRow = BM.createAusgabenRechnung();
...
function updateNameFromDataAndTemplate(ausgabeRow, template) {
...
function neuenBewirtungsbelegEintragen(beleg, belegWoerter, monat, BM) {
    if (belegWoerter.length > 2) {
        var neuerBewirtungsbelegRow = BM.createBewirtungsbeleg();
...
function neueAusgabeEintragen(beleg, belegWoerter, datum, BM) {
    var neueAusgabeRow = BM.createAusgabenRechnung();
    neueAusgabeRow.setFileId(beleg.getId());
    neueAusgabeRow.createLink(beleg.getId(), beleg.getName());
    neueAusgabeRow.setDatum(datum);
    if (belegWoerter.length > 2) {
        ...
        if (belegName.indexOf("19%") != -1)
            prozent = "19%";
        if (belegName.indexOf("7%") != -1)
            prozent = "7%";
        ...
        var gegenkonto = 'bar';
        var bezahltAm = datum;
        if (belegName.indexOf("bar") != -1 || belegName.indexOf("Bar") != -1)
            gegenkonto = "bar";
        if (belegName.indexOf("auf Rechnung") != -1 || belegName.indexOf("Auf Rechnung") != -1) {
            gegenkonto = "auf Rechnung";
            bezahltAm = "";
        }
        if (belegName.indexOf("mit Karte") != -1 || belegName.indexOf("Mit Karte") != -1) {
            gegenkonto = "mit Karte";
            bezahltAm = "";
        }
        if (belegName.indexOf("Verbindlichkeiten Umsatzsteuer") != -1) {
            gegenkonto = "Verbindlichkeiten Umsatzsteuer";
            bezahltAm = "";
        }
