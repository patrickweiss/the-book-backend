function SimbaExportErstellen(rootFolderId) {
    ...
    try {
        ...
        for (var index in a.csvCache.dataArray) {
            ...
            if (buchungRow.getValue("Datum") < a.geschaeftsjahr)
                ebBuchungAnpassen(a, buchungRow);
        }
        ...
        for (var index in a.csvCache.dataArray) {
            ...
            if (csvRow.getValue("Exportgruppe") != "") {
                if (buchungenCSV[csvRow.getValue("Exportgruppe")] === undefined)
                    buchungenCSV[csvRow.getValue("Exportgruppe")] = "Datum;Betrag;Konto (Soll);Gegenkonto (Haben);Buchungstext;Automatiksperre\n";
...
function ebBuchungAnpassen(a, csvRow) {
    ...
    if (parseInt(csvRow.getValue("SKR03 (Soll)")) >= 2000 && parseInt(csvRow.getValue("SKR03 (Soll)")) !== 2868)
        csvRow.setValue("SKR03 (Soll)", 9000);
    if (parseInt(csvRow.getValue("SKR03 (Haben)")) >= 2000)
        csvRow.setValue("SKR03 (Haben)", 9000);
    if (parseInt(csvRow.getValue("SKR03 (Soll)")) == 1766)
        csvRow.setValue("SKR03 (Soll)", 9000);
    if (parseInt(csvRow.getValue("SKR03 (Haben)")) == 1766)
        csvRow.setValue("SKR03 (Haben)", 9000);
}
...
function ausgabenCSV(a) {
    ...
    if (a.ausgabenCache.getRowByIndex(0).getValue("Datum") == "")
        return;
...
function bewirtungsbelegeCSV(a) {
    ...
    if (a.bewirtungsbelegeCache.getRowByIndex(0).getValue("Datum") == "")
        return;
    // @ts-ignore
...
function verpflegungsmehraufwendungenCSV(a) {
    ...
    // @ts-ignore
    a.quelle = saw.linkFormula(ausgabenID);
    if (a.verpflegungsmehraufwendungenCache.getRowByIndex(0).getValue("Datum") == "")
        return;
...
function rechnungenCSV(a) {
    ...
    if (a.rechnungenCache.getRowByIndex(0).getValue("Datum") == "")
        return;
...
function gutschriftenCSV(a) {
    ...
    if (a.gutschriftenCache.getRowByIndex(0).getValue("Datum") == "")
        return;
...
function umbuchungenCSV(a) {
    ...
    if (a.umbuchungenCache.getRowByIndex(0).getValue("Datum") == "")
        return;
    ...
    for (var index in a.umbuchungenCache.dataArray) {
        ...
        var umbId = buchungRow.getValue("ID");
...
function bankbuchungenCSV(a) {
    ...
    if (a.bankbuchungenCache.getRowByIndex(0).getValue("Datum") == "")
        return;
    a.quelle = a.bankkontenLinkFormula;
...
function abschreibungenCSV(a) {
    ...
    if (a.abschreibungenCache.getRowByIndex(0).getValue("Datum") == "")
        return;
    a.quelle = a.ausgabenLinkFormula;
...
function getSKR03ifConfigured(a, konto) {
    ...
    if (kontoRow.getValue("SKR03") != "")
        return kontoRow.getValue("SKR03");
    else
        return konto;
}
